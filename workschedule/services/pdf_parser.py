...
- CRITICAL — a shift's date comes ONLY from its own individual per-day date label (the specific date positioned immediately next to or above that shift's own time/detail text), never from any other date found elsewhere in the image. In particular, the document's title/header often states an overall date RANGE for the whole document (e.g. "Selected Date Range Feb 25, 2026 - Mar 15, 2026") — this describes the document's scope only. Never use that range's start date, end date, or any date drawn from it as an individual shift's date, even if a specific shift's own date label is nearby or looks similar. Trace each shift to its own, distinct date label before recording it.
...
- CRITICAL — a shift's date comes ONLY from its own individual per-day date label (the specific date positioned immediately next to or above that shift's own time/detail text), never from any other date found elsewhere in any image. In particular, the document's title/header often states an overall date RANGE for the whole document (e.g. "Selected Date Range Feb 25, 2026 - Mar 15, 2026") — this describes the document's scope only. Never use that range's start date, end date, or any date drawn from it as an individual shift's date, even if a specific shift's own date label is nearby or looks similar. Trace each shift to its own, distinct date label before recording it.
- If a section explicitly states there's nothing scheduled (e.g. "No shifts are scheduled within the timeframe"), emit ZERO events for that section — never invent a placeholder or all-day event to represent it.
